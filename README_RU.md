...
flowchart LR
    ...
    APP --> CH[Публичный канал<br>история треков]
```
...
- может временно назначать выбранный публичный канал личным каналом профиля, хранить там ограниченную историю треков и восстанавливать прежний канал;
...
- удалить публикации приложения из канала;
- изменить настройки;
...
| Треков в истории | Максимальный размер LRU-истории для выбранных назначений |
| Параллельных загрузок | Число фоновых загрузчиков для режима `mixed` |
| Тема интерфейса | Переключает современную тёмную и светлую темы; по умолчанию используется тёмная |
| Куда отправлять музыку | `Профиль` сохраняет историю в Music on Profile; `Личный канал` публикует её в канале; `Профиль + канал` делает и то и другое без повторного скачивания |
...
В режиме **Личный канал** мастер аналогично просит временно выбрать канал через **Telegram → Изменить профиль → Личный канал**, считывает его ID и возвращает исходный канал. При `nowplaying` приложение назначает выбранный канал личным и хранит в нём до **Треков в истории** аудиопостов (по умолчанию 20). При достижении лимита удаляется самый давно прослушанный трек; повтор уже сохранённого трека переносит его пост в конец без повторного скачивания. После подтверждённого idle, паузы или штатного выхода все эти посты удаляются, а прежний личный канал восстанавливается. Музыка при этом не добавляется в Music on Profile и Saved Messages не используется. Emoji status можно независимо включить и для этого режима. Ручные изменения личного канала и emoji status во время прослушивания не перезаписываются.

Режим **Профиль + канал** использует один подготовленный аудиофайл для обоих назначений: одинаковая ограниченная LRU-история сохраняется и в Music on Profile, и в канале. При idle все канальные посты удаляются всегда, а музыка профиля — только если включена настройка **Удалять музыку профиля, когда nowplaying исчез**.
...
├── state.json           # созданные сообщения и ожидающее восстановление профиля Telegram
...
Предыдущий emoji status сохраняется в `state.json`. После принудительного завершения процесса приложение восстановит его при следующем запуске, если текущим всё ещё остаётся emoji прослушивания, установленный приложением. Ручное изменение статуса из другого клиента Telegram не перезаписывается.
</details>

<details>
<summary><strong>После аварийного завершения остался музыкальный личный канал</strong></summary>

Предыдущий личный канал, как и emoji status, сохраняется в `state.json`. После принудительного завершения приложение восстановит его при следующем запуске, только если личным всё ещё назначен канал приложения. Ручные изменения сохраняются, а оставшиеся аудиопосты удаляются после следующего подтверждённого idle в том же канальном режиме.
</details>
...
Каждый push в `main` запускает CI без публикации GitHub Release. Для выпуска версии используйте команду выше: скрипт при необходимости создаст release-коммит, стабильный тег `v1.2.0` и отправит их в GitHub. Push этого тега запускает release workflow, сверяет версию и публикует обычный GitHub Release с wheel и ZIP исходников. Подробности находятся в [CONTRIBUTING_RU.md](CONTRIBUTING_RU.md) и [CONTRIBUTING.md](CONTRIBUTING.md).
